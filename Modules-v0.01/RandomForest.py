...
class RandomForestAlgorithm():
	...
	def random_forest_regressor(self, n_jobs, estimators, step_factor, axis_step):
		...
		for i in range(0, self.axis_step):
			...
			# self.acc_train = mean_squared_error(self.y_train, self.train_predictions)
			# self.acc_test = mean_squared_error(self.y_test, self.test_predictions)
			self.estimators[i] = self.step_factor*(i + 1)
			self.rf_acc_train[i] = self.acc_train
			self.rf_acc_test[i] = self.acc_test
	...
	def graph(self, train_accuracy, test_accuracy, estimators, chart = 'Test'):
